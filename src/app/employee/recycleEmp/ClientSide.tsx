...
export default function AdminRecyclePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const wasteRes = await fetch("/api/employee/waste-types");
      const wasteData = await wasteRes.json();

      const catRes = await fetch("/api/employee/recycle-categories");
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(`/api/employee/waste-types/${selectedWaste.id}`, {
        method: "PATCH",
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch("/api/employee/waste-types", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: addForm.name,
